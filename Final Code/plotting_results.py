...
def plot_subplot_episode_reward(result,n,axs,i,color,label):
    ...
    axs[i].plot(mar_max, color, linewidth=0.5)
    axs[i].plot(mar_min, color, linewidth=0.5)
...
def plot_only_return(result,n,axs,color,label=None,threshold=None):
    ...
    axs.plot(mar_min, color, linewidth=0.5)
    ...
    print('Maximum mean:'+str(mar_mean.max()),'\n Episode:'+str(maximum_idx),'\n std:'+str(ma_result_std[maximum_idx]))
    if threshold is not None:
        first_time_solved_idx=np.where(mar_mean>threshold)
        print('System solved at'+str(first_time_solved_idx)+'episode')
